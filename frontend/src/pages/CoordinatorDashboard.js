...
// ─── ESCALATION MODAL ─────────────────────────────────────────────────────────
function EscalateModal({ complaint, targetRole, onClose, onConfirm }) {
  const [reason, setReason] = useState('');

  const reasons = [
    'Issue persists despite staff intervention',
    'Requires departmental authority to resolve',
    'Policy violation — needs HOD review',
    'Multiple students affected — urgent',
    'Infrastructure issue beyond staff scope',
    'Other (specify below)',
  ];
  ...
  return (
    <div style={modal.overlay}>
      <div style={modal.box}>
        <h3 style={modal.title}>⚡ Escalate to {targetRole}</h3>
        <p style={modal.sub}>
...
function CoordinatorDashboard() {
  ...
  const handleEscalate = async (id, reason) => {
    try {
      ...
      setMessage('⚡ Complaint escalated to HOD!');
      fetchComplaints();
  ...
  // Coordinator sees complaints escalated to level 1 (from support staff)
  const allEscalated = complaints.filter(c => c.escalation_level >= 1);
  const openEscalated = allEscalated.filter(c => c.status === 'ESCALATED');
  const inProgress = allEscalated.filter(c => c.status === 'IN_PROGRESS');
  const resolved = allEscalated.filter(c => c.status === 'RESOLVED');
  const furtherEscalated = allEscalated.filter(c => c.escalation_level >= 2);

  const tabs = [
    { id: 'all', label: '📋 All Escalated', count: allEscalated.length },
    { id: 'open', label: '🔴 Needs Action', count: openEscalated.length },
    { id: 'inprogress', label: '⏳ In Progress', count: inProgress.length },
    { id: 'resolved', label: '✅ Resolved', count: resolved.length },
    { id: 'hod', label: '⚡ Sent to HOD', count: furtherEscalated.length },
  ];
    ...
    <div style={styles.container}>
      ...
      <div style={styles.sidebar}>
        <div style={styles.sidebarLogo}>
          ...
          <div>
            ...
            <div style={styles.sidebarLogoSub}>Coordinator</div>
          </div>
        </div>
        ...
        <div style={styles.sidebarStats}>
          <div style={styles.sidebarStatItem}>
            <span style={styles.sidebarStatLabel}>Escalated to me</span>
            <span style={{ ...styles.sidebarStatValue, color: '#f87171' }}>{allEscalated.length}</span>
          </div>
          <div style={styles.sidebarStatItem}>
          ...
          <div style={styles.sidebarStatItem}>
            <span style={styles.sidebarStatLabel}>Sent to HOD</span>
            <span style={{ ...styles.sidebarStatValue, color: '#c084fc' }}>{furtherEscalated.length}</span>
          </div>
        </div>
        ...
        <div style={styles.sidebarUser}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={styles.sidebarUserRole}>Coordinator</div>
          </div>
      ...
      <div style={styles.main}>
        <div style={styles.header}>
          <div>
            ...
            <p style={styles.headerSub}>
              Review complaints escalated by Support Staff and take action.
            </p>
          </div>
        ...
        <div style={styles.statsGrid}>
          {[
            { label: 'Total Escalated', value: allEscalated.length, color: '#6366f1', bg: '#eef2ff', icon: '📋' },
            { label: 'Needs Action', value: openEscalated.length, color: '#e11d48', bg: '#fff1f2', icon: '🔴' },
            ...
            { label: 'Sent to HOD', value: furtherEscalated.length, color: '#9333ea', bg: '#faf5ff', icon: '⚡' },
          ].map(s => (
        ...
        {/* INFO BOX — explains the coordinator's role */}
        <div style={styles.infoBox}>
          <strong>ℹ️ Your Role:</strong> You receive complaints escalated by Support Staff.
          You can resolve them directly or escalate further to the HOD if needed.
        </div>
              ...
              <tbody>
                {currentList.map((c, i) => (
                  <tr key={c.id} style={{ backgroundColor: i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={styles.td}>
                      <span style={styles.levelBadge}>Level {c.escalation_level}</span>
                    </td>
                    ...
                    <td style={styles.td}>
                      <div style={styles.actionRow}>
                        {c.status !== 'RESOLVED' && c.escalation_level < 2 && (
                          <>
                            <button
                        ...
                        {c.escalation_level >= 2 && (
                          <span style={styles.sentBadge}>Sent to HOD</span>
                        )}
                      </div>
                    </td>
                  </tr>
...
const styles = {
  ...
  avatar: { width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#7c3aed', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '700', fontSize: '15px', flexShrink: 0 },
  ...
  infoBox: { backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', padding: '12px 16px', color: '#1d4ed8', fontSize: '13px', marginBottom: '20px' },
  ...
  sentBadge: { backgroundColor: '#faf5ff', color: '#9333ea', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' },
...
const modal = {
  ...
  reasonItemActive: { padding: '10px 14px', borderRadius: '8px', border: '1px solid #7c3aed', backgroundColor: '#faf5ff', cursor: 'pointer', fontSize: '13px', color: '#7c3aed', fontWeight: '600' },
